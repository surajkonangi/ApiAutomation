...
public class GetByIdApiWiremockApiKey {
	...
	public void get() throws URISyntaxException {
		...
		Response response = RestAssured.given().baseUri("http://localhost:8080").header("Content-Type", "application/json").header("Authentication", keyvalue)
				.accept(ContentType.JSON).when().get("/api/employees/1").then().assertThat().statusCode(200).and()
				.body("data.employee_name", Matchers.equalTo("suraj konangi")).log().all().extract().response();
